...
namespace ArcadeJets {
	...
	public class JetMovement : MonoBehaviour {
		[Tooltip("Controller for the jet.")] public StickInput input;

		[Tooltip("How powerfully the plane can maneuver in each axis.\n\nX: Pitch\nY: Yaw\nZ: Roll")]
		public Vector3 turnTorques = new Vector3(60.0f, 200.0f, 1000.0f);
		...
		private void FixedUpdate() {
			...
			float brakePower =
				brakeDrag * Mathf.InverseLerp(StickInput.ThrottleNeutral, StickInput.ThrottleMin, throttleTarget);
			...
			throttleTrue = Mathf.MoveTowards(throttleTrue, throttleTarget,
				((acceleration + brakeAccel) / FORCE_MULT) * Time.deltaTime);
